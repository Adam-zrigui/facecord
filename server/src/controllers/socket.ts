...
type WebSocketType = {
  ...
  message(message: Message, conversation: Conversation, myUserId: string): Promise<void>;
...
class WebSocket implements WebSocketType {
  constructor(private socket: Socket, private prisma: PrismaClient) {
    ...
    this.conversationChange = this.conversationChange.bind(this);
  }
  ...
  async message(message: Message, conversation: Conversation, myUserId: string) {
    ...
    const filteredMyUserId = conversation.participants.filter((id : string) => id !== myUserId);
    const otherUserId = filteredMyUserId[1];
